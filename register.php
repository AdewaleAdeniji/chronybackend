<?php
 	...
 	//$dat = '{"email":"email@dev.com","username":"username","password":"password"}';
	...
	if(checkup($data)){ //
		...
		if(emailExists($email)){
			...
			$link = '<a href="https://chrony.netlify.app/verify#'.$verificationToken.'">Verify my account</a>';
			$body = "Thanks for signing up on Chrony,For security purposes please click on this link to verify your email address".$link;
			mailme("Verify your Email Address:Chrony",$email,$username,$body);
			$userid = $email;
			$expiry = expiry();//this function returns the token expiry/session 
			$sql = query("INSERT INTO chronyusers(username,useremail,userpassword,status) VALUES('$username','$email','$newpassword','1') ");
